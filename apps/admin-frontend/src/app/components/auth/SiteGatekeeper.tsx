...
/**
 * SiteGatekeeper - Access Visibility Gatekeeper Component (Admin Panel)
 * 
 * Provides a simple passcode lock screen before displaying the admin portal.
 * Controlled via environment variables:
 *   VITE_ENABLE_GATEKEEPER="true"  (or "false" to bypass/remove)
 *   VITE_GATEKEEPER_PASSCODE="123456"  (default PIN)
 */
const SiteGatekeeper: React.FC<Props> = ({ children }) => {
  // Flag to check if Gatekeeper feature is enabled (defaults to true unless explicitly 'false')
  const isEnabled = (import.meta as any).env.VITE_ENABLE_GATEKEEPER !== 'false';

  // Passcode configured in .env or default '123456'
  const VALID_PASSCODE = (import.meta as any).env.VITE_GATEKEEPER_PASSCODE || '123456';

  const [isUnlocked, setIsUnlocked] = useState<boolean>(() => {
    if (!isEnabled) return true;
    try {
      return sessionStorage.getItem('mhn_admin_gatekeeper_unlocked') === 'true';
    } catch (e) {
      return false;
    }
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      sessionStorage.setItem('mhn_admin_gatekeeper_unlocked', 'true');
    } catch (e) {}
    setIsUnlocked(true);
  };
    ...
    <div style={styles.overlay}>
      <div style={styles.card}>
        <div style={styles.header}>
          <div style={styles.badge}>🔒 PRIVATE OPERATIONS PORTAL</div>
          <h1 style={styles.title}>MaiHoonNa Admin</h1>
          <p style={styles.subtitle}>
            This environment is password protected. Enter the access passcode to view the portal.
          </p>
        </div>
        ...
        <div style={styles.footer}>
          <span>MaiHoonNa Operations</span>
